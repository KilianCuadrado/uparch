...
async def create_folder(
    folder: FolderCreate, current_user: dict = Depends(getCurrentUser)
):
    """
...
async def list_folders(
    parent_id: Optional[int] = None, current_user: dict = Depends(getCurrentUser)
):
    """
...
async def delete_folder(folder_id: int, current_user: dict = Depends(getCurrentUser)):
    """
    Eliminar un folder.
...
async def rename_folder(
    folder_id: int, new_name: str, current_user: dict = Depends(getCurrentUser)
):
    """Renombrar un folder"""
    ...
    try:
        ...
        # Renombrar
        cursor.execute(
            "UPDATE folders SET name = ? WHERE id = ?", (new_name.strip(), folder_id)
        )
        conn.commit()
        conn.close()
